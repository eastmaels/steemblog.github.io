
---
title: 'CoinTools Update: Show Full Cryptocurrency Details by Click or Startup, Add Language Handlers'
permlink: cointools-update-show-full-cryptocurrency-details-by-click-or-startup-add-language-handlers
catalog: true
toc_nav_num: true
toc: true
position: 9999
date: 2018-02-20 23:20:54
categories:
- utopian-io
tags:
- utopian-io
- cryptocurrency
- steemstem
- programming
- software
thumbnail: https://res.cloudinary.com/hpiynhbhq/image/upload/v1519168554/t4zyoxxjahhltvytrykh.png
sidebar:
    right:
        sticky: true
widgets:
    -
        type: toc
        position: right
---


## Introduction
CoinTools is a handy gadget to Chrome browser that you can launch easily to view the information of cryptocurrency. 

## Previous Contributions
- [CoinTools v0.0.5: Update: Cryptocurrency Converter Calculator, Support Coin Symbols and Add More Localization](https://helloacm.com/cointools-update-cryptocurrency-converter-calculator-support-coin-symbols-and-add-more-localization/)
- [CoinTools v0.0.4:  Conversion Between Two Fiat or Fiat-Coin + 24 Hour Cap Chart](https://helloacm.com/cointools-v0-0-4-conversion-between-two-fiat-or-fiat-coin-24-hour-total-market-cap-chart/)
- [CoinTools v0.0.3: Adding Total Market Cap USD Chart, Localization and Stock Price Emoji](https://helloacm.com/cointools-v0-0-3-adding-total-market-cap-usd-chart-localization-and-stock-price-emoji/)
- [v0.0.2 Cryptocurrency Conversion + UI Localization](https://helloacm.com/cointool-v0-0-2-cryptocurrency-conversion-ui-localization/)
- [v0.0.1 Introduction to CoinTools! A Cryptocurrency Chrome Extension](https://helloacm.com/introduction-to-cointools-a-cryptocurrency-chrome-extension/)

## Technology Stacks
Javascript that runs in [Chrome](https://helloacm.com/how-to-enable-inline-chrome-extension-installation-in-chrome-browser/).

## Github
https://github.com/DoctorLai/CoinTools

## Chrome Webstore
It is available online at Chrome Webstore:
https://chrome.google.com/webstore/detail/coin-tools/fmglcggbdcbkpkfapngjobfeakehpcgj

## v0.0.6 Feature
Along with bug fixes, minor tweaks and code refactoring, [the version 0.0.6](https://helloacm.com/cointools-update-show-full-cryptocurrency-details-by-click-or-startup-add-language-handlers/) has the following updates:

1. You can enter coin ID in settings that will show you full details of that particular cryptocurrency in the converter console.
2. You can click the coin button at ranking table to show full details
3. Minors: Add a clear console button, Make console readonly, add language hanlders (support traditional Chinese and Portuguese)

## Commits
**[Here](https://github.com/DoctorLai/CoinTools/commit/8914a77235054a98d1c19df3d9d0991887cf5876)**

## Roadmap of CoinTools
1. real-time graphs
2. search cryptocurrency
3. historical data

## Screenshots
You can enter coin ID in settings that will show you full details of that particular cryptocurrency in the converter console.
![image.png](https://res.cloudinary.com/hpiynhbhq/image/upload/v1519168554/t4zyoxxjahhltvytrykh.png)

![image.png](https://res.cloudinary.com/hpiynhbhq/image/upload/v1519168577/gzbkusszhrp3zarbaybw.png)

You can click the coin button at ranking table to show full details
![image.png](https://res.cloudinary.com/hpiynhbhq/image/upload/v1519168590/tz0rc5wjset64iq7usiz.png)

![image.png](https://res.cloudinary.com/hpiynhbhq/image/upload/v1519168605/cr7hb6wzotrydewzaj8e.png)

Support Traditional Chinese and Portugues [**Tasks Requests: UI Translations for CoinTools
**](https://steemit.com/utopian-io/@justyy/ui-translations-for-cointools)
![image.png](https://res.cloudinary.com/hpiynhbhq/image/upload/v1519168642/bpgysy4guiajat3ave6p.png)

# Javascript to handle conversion
```
// conversion
const processConversion = (s) => {
    let arr = s.trim().split("\n");
    for (let i = 0; i < arr.length; i ++) {
        let pair = arr[i].split(" ");
        if (pair.length == 2) {
            let a = pair[0].trim().toLowerCase();
            let b = pair[1].trim().toLowerCase();
            var pat = /^[a-zA-Z\-]+$/;
            if (pat.test(a) && pat.test(b)) {
                let dom = $('div#conversion_results');
                let dom_id = "convert_" + removeInvalid(a) + "_" + removeInvalid(b);
                dom.append('<div id="' + dom_id + '"> </div>');
                getConversion(a, b).then(x => {
                    $('div#' + dom_id).html("<h4>1 " + a.toUpperCase() + " = <span class=yellow>" + x + "</span> " + b.toUpperCase() + "</h4>");
                });
            }
        } else if (pair.length == 1) {
            let a = pair[0].trim().toUpperCase();
            let currency = $('select#currency').val();
            var pat = /^[a-zA-Z\-]+$/;
            if (pat.test(a)) {
                if (a in coinmarkcap) {
                    a = coinmarkcap[a];                    
                }                
                let api = 'https://api.coinmarketcap.com/v1/ticker/' + a.toLowerCase() + '/';
                if (currency != '') {
                    api += '?convert=' + currency;
                }                
                fetch(api, {mode: 'cors'}).then(validateResponse).then(readResponseAsJSON).then(function(result) {
                    result = result[0];
                    $('textarea#convert_result').append(getCoinReport(result, currency));
                }).catch(function(error) {
                    logit('Request failed: ' + api + ": " + error);                    
                });                
            }
        }
    }
}
```

# License
[MIT](https://github.com/DoctorLai/CoinTools/blob/master/LICENSE)

# Contribution Welcome
Github: https://github.com/DoctorLai/CoinTools/
1. Fork it!
2. Create your feature branch: `git checkout -b my-new-feature`
3. Commit your changes: `git commit -am 'Add some feature'`
4. Push to the branch: `git push origin my-new-feature`
5. Submit a pull request.

# Chrome Webstore
Install the [CoinTools](https://chrome.google.com/webstore/detail/coin-tools/fmglcggbdcbkpkfapngjobfeakehpcgj) Now!

<br /><hr/><em>Posted on <a href="https://utopian.io/utopian-io/@justyy/cointools-update-show-full-cryptocurrency-details-by-click-or-startup-add-language-handlers">Utopian.io -  Rewarding Open Source Contributors</a></em><hr/>

- - -

This page is synchronized from the post: [CoinTools Update: Show Full Cryptocurrency Details by Click or Startup, Add Language Handlers](https://steemit.com/@justyy/cointools-update-show-full-cryptocurrency-details-by-click-or-startup-add-language-handlers)
