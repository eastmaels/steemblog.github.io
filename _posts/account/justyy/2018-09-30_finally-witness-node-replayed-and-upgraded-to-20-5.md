
---
title: 'Finally Witness Node Replayed and Upgraded to 20.5!'
permlink: finally-witness-node-replayed-and-upgraded-to-20-5
catalog: true
toc_nav_num: true
toc: true
position: 9999
date: 2018-09-30 09:05:33
categories:
- witness-category
tags:
- witness-category
- witness-update
- busy
- steem
- witness
thumbnail: 'https://ipfs.busy.org/ipfs/QmYSoBGz4eAGJyPsf9MfyiTXicHdyfJxaTAtYhzHKZUrXZ'
sidebar:
    right:
        sticky: true
widgets:
    -
        type: toc
        position: right
---


Witness Node Replayed and Upgraded to 20.5 beautifully without any problems - practice makes perfect - I have spent lots of energy in the last few days building, replaying and switching between main and backup nodes so basically nothing new except the version from 0.20.2 to 0.20.5.

![image.png](https://ipfs.busy.org/ipfs/QmYSoBGz4eAGJyPsf9MfyiTXicHdyfJxaTAtYhzHKZUrXZ)

My witness information: https://helloacm.com/tools/steemit/witness-data/justyy
![image.png](https://ipfs.busy.org/ipfs/QmTRRzo1oAkwDrC9WpQLYc5zdgqS5RJCUCjPvTKQZ91wuS)

> A Steem Witness is a person who operates a witness server (which produces blocks), and publishes a price feed of STEEM/USD to the network. Witnesses' computers in the Steem network have a job that is to make 'blocks' and add them to the blockchain and also to store the complete blockchain data.

##  [Vote for me](https://steemconnect.com/sign/account_witness_vote?approve=1&witness=justyy) or [Set me as a witness Proxy](https://v2.steemconnect.com/sign/account-witness-proxy?proxy=justyy&approve=1) - Every vote counts! - Thank you!

## Your Vote is much appreciated, and every vote counts.
Check out [My Witness Page](https://helloacm.com/tools/steemit/witness-data/justyy)

## Support me and my work as a witness - [witness thread](https://steemit.com/witness-category/@justyy/justyy-just-another-witness) by 
1. voting me [here](https://steemconnect.com/sign/account_witness_vote?approve=1&witness=justyy), or
2. voting me as [a witness proxy](https://v2.steemconnect.com/sign/account-witness-proxy?proxy=justyy&approve=1) - let @justyy represent you.

Thank you! **Some of My Contributions: [SteemIt Tutorials, Robots, Tools and APIs](https://helloacm.com/tools/steemit/)**

- - -

This page is synchronized from the post: ['Finally Witness Node Replayed and Upgraded to 20.5!'](https://steemit.com/@justyy/finally-witness-node-replayed-and-upgraded-to-20-5)
